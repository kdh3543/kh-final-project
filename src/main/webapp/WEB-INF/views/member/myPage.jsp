                          ...
                          <a class="dropdown-item" id="search-dropdown" href="javascript:void(0);"
                            ...
                            <nav class="top-nav">
                              <div class="nav nav-tabs" id="nav-tab1" role="tablist">
                                ...
                                <button class="nav-link active" id="nav-search-tab" data-bs-toggle="tab"
                                  data-bs-target="#nav-search" type="button" role="tab" aria-controls="nav-search"
                                  aria-selected="true">최근검색어</button>


                                <button class="nav-link" id="nav-hotkeyword-tab" data-bs-toggle="tab"
                                  data-bs-target="#nav-hotkeyword" type="button" role="tab"
                                  aria-controls="nav-hotkeyword" aria-selected="false">인기검색어</button>
                            ...
                            <div class="tab-content" id="nav-tabContent">
                              <div class="tab-pane fade show active" id="nav-home" role="tabpanel"
                                ...
                                <!-- 내용 채워넣기 -->
                                <div id=text>
                                  <button type=button id=delBtn class="dropdown-item" style="display: inline">
                                    <b>
                                      <h6>검색어 전체삭제</h6>
                                    </b>
                                  </button>
            ...
            <nav>
              <div class="nav nav-tabs" id="nav-tab" role="tablist">
                <button class="nav-link active" id="nav-profile-tab" data-bs-toggle="tab" data-bs-target="#nav-profile"
                  type="button" role="tab" aria-controls="nav-profile" aria-selected="true">회원정보수정</button>
                <button class="nav-link" id="nav-sell-tab" data-bs-toggle="tab" data-bs-target="#nav-sell" type="button"
                  role="tab" aria-controls="nav-sell" aria-selected="false">판매내역</button>
                ...
                <button class="nav-link" id="nav-like-tab" data-bs-toggle="tab" data-bs-target="#nav-like" type="button"
                  role="tab" aria-controls="nav-like" aria-selected="false">찜</button>
                <button class="nav-link" id="nav-following-tab" data-bs-toggle="tab" data-bs-target="#nav-following"
                  type="button" role="tab" aria-controls="nav-following" aria-selected="false">팔로워</button>
                <button class="nav-link" id="nav-follower-tab" data-bs-toggle="tab" data-bs-target="#nav-follower"
                  type="button" role="tab" aria-controls="nav-follower" aria-selected="false">팔로우
                </button>
              </div>
            </nav>
            <div class="tab-content" id="nav-tabContent">
                ...
                <form action="/member/updateInfo" method="post" enctype=multipart/form-data>
                  <div class="profile-content">
                    <div class="profile-img">
                      <label for="imgfile" class="del-button img-up"> <input type="file" id=imgfile name="file"
                    ...
                    <div class="profile-content-section">
                      <table class="profile-table">
                        ...
                        <tr>
                          <td>비밀번호</td>
                          <!-- ^^^ -->
                          <td><input type="hidden" class="input Password" id="pw" name="pw" value="${dto.pw}"> <input
                              type="text" class="input Password" id="cpw" name="pw"
                              placeholder="최소 한개의 문자, 한개의 숫자 , 한개의 특수 문자를 포함한 8~20자리" value="비밀번호를 변경하시려면 버튼을 눌러주세요"
                              pattern="^(?=.*[A-Za-z])(?=.*\d)(?=.*[$@$!%*#?&])[A-Za-z\d$@$!%*#?&]{8,20}$" required
                              disabled>


                            <button type="button" class=" btn btn-lg btn-light" id="pwChange">비밀번호 변경</button>
                          </td>
                        </tr>
                        <tr>
                          <td>이름</td>
                        ...
                        <tr>
                          <td>전화번호</td>
                          <td><input type="text" class="inputPhone" id="phone" name="phone" value="${dto.phone }"
                              readonly required>
                            <button type="button" class=" btn btn-lg btn-light" id="phoneChange">전화번호 변경</button>
                          </td>
                        </tr>
                        <tr>
                          <td>우편변호</td>
                          <td><input type="text" class="inputZipcode" id="inputZipcode" name="zipcode"
                              value="${dto.zipcode }" required>
                            <button type="button" class=" btn btn-lg btn-light" id="findAddress">주소 검색</button>
                          </td>
                        </tr>
                        <tr>
                          <td>주소1</td>
                          <td><input type="text" class="inputAddress1" id="inputAddress1" name="address1"
                              value="${dto.address1 }" readonly required></td>
                        </tr>
                        <tr>
                          <td>주소2</td>
                          <td><input type="text" class="inputAddress2" id="address2" name=address2
                              value="${dto.address2 }" required></td>
                        </tr>
                        <tr>
                          <td>거래선호지역</td>
                          <td><input type="text" class="inputlocation" id="prefer_location" name=prefer_location
                              value="${dto.prefer_location }" required></td>
                        </tr>
                        <tr>
                          <td>잔여포인트</td>
                          <td><input type="text" class="" id="remainPoint" name=cash value="${dto.cash}" disabled></td>
                        </tr>
                        <tr>
                          <td colspan="2">
                            <button type="submit" class=" btn btn-lg btn-light" id="updateInfo">정보수정</button>
                            <button type="button" class=" btn btn-lg btn-light" id="deleteInfo">회원탈퇴</button>
                          <td>
                        </tr>
                      </table>
                    </div>
                    ...
                    <c:forEach var="i" items="${ilist}" varStatus="statusI">
                      <c:forEach var="f" items="${flist}" varStatus="statusF">
                        ...
                        <c:if test="${statusI.index eq statusF.index }">
                          ...
                          <tr>
                            ...
                            <td><select class="sellBoxes">
                                ...
                                <option value="Y" <c:if test="${i.deal eq 'Y' }">selected
                        </c:if>>판매중</option>
                        ...
                        <td><a href="/items/itemsModify?iseq=${i.iseq}">
                          ...
                          <button type="button" class=" btn btn-lg btn-light deleteBtn">삭제</button> </a>
                        </td>
                    ...
                    <c:forEach var="i" items="${buyIlist}" varStatus="statusI">
                      <c:forEach var="f" items="${buyFlist}" varStatus="statusF">
                        <c:if test="${i.iseq == f.parentSeq}">
                          <tr>
                            <td><a href=/items/itemsDetail?iseq=${i.iseq}><img src="${f.sysName}"></a></td>
                            <c:if test="${i.deal2 =='R'}">
                              <td>구매중</td>
                            </c:if>
                            <c:if test="${i.deal2 =='Y'}">
                            ...
                            <th>
                              <c:choose>
                                <c:when test="${i.deal2 == 'Y'}">
                                  <button class=" btn btn-lg btn-light " id="otherConfirmBtn">구매확정</button>
                                </c:when>
                                <c:otherwise>
                                  <button class="btn btn-lg btn-light buyOk" id="confirmBtn">구매확정</button>
                                  <button class="btn btn-lg btn-light buyCancel" id="cancleBtn">구매취소</button>

                                </c:otherwise>
                              </c:choose>
                              <!-- <button class=" btn btn-lg btn-light" id="readReviewBtn" hidden>거래완료</button> -->
                            </th>
                      ...
                      <%-- <a id="rmvStyle" href="/items/itemsDetail?iseq=${wl.iseq}"> --%>
                        ...
                        <div class="like-section">
                          ...
                          <div class="like-right">
                            ...
                            <a href="javascript:void(0);" id="${wl.sellerID}" class="dellist on"> <input type=hidden
                                id="iseq" value="${wl.iseq}">
                              <button type=button value="X" class="deleteXL">
                                <i class="far fa-window-close fa-2x"></i>
                              </button>
                              <%-- <a href="#" id="${wl.sellerID}" class="dellist on">
                                <input type=hidden id="iseq" value="${wl.iseq }">
                                <input type=button value="X" class="deleteXL" style="background-color: #24a6a4"> --%>
                    ...
                    <c:otherwise>
                      <div class="following-section">
                        ...
                        <div class="following-right">
                          ...
                          <a href="#" fid="${f.id}" class="btnFollow followed"> <input type=button value="팔로우"
                              class="unfollow btn-lg btn-light" style="background-color: #24a6a4">
                          </a>
