   ...
   public String moveChatRoom(String sellerId, int productId, String productName, int roomId, String sysName, Model model) {
      ...
      for(int i =0; i<list.size();i++) {
         if(cService.selectLastDate(list.get(i).getRoomId())==null) {
        	 System.out.println("현재 1시각은: "+DateParseUtils.nowDate(time));
            list.get(i).setLatestDate(DateParseUtils.nowDate(time));            
         }else {      
        	 System.out.println("현재 2시각은: "+cService.selectLastDate(list.get(i).getRoomId()));
         ...
         // chat 마지막 채팅글 세팅
         // 채팅 길이 넘쳤을 때 조절
         if(cService.selectLastTalk(list.get(i).getRoomId())==null) {
            
         }else {
            if(cService.selectLastTalk(list.get(i).getRoomId()).length()>15) {
               String shortMessage = cService.selectLastTalk(list.get(i).getRoomId()).substring(0,15);
               list.get(i).setLastMessage(shortMessage);
            }else {
